...
>
> **第二批（待开工）**：跨来源近似重复合并（现在只在同一 `sourceId` 内做，网页转载/网站与工作区同文档会留两条并可能被判伪矛盾）；抓取篇数/总字数上限 + 生成汇总显式告知；无 Provider 时跳过网页抓取并改用闸门后的材料（现在会抓完再把**全部宽召回段落**写进汇编）。
>
...
>
> **第三批（待开工）**：网页材料集合在首次生成时落定（避免每次生成引入新文章、污染版本差异）；生成前给出「本次将使用的网页材料清单」并允许排除；来源小卡显示抓取时间并优先展示本地快照（线上 URL 会改版/撤稿）。
>
