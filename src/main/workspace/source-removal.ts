...
export function getSourceRemovalStats(sourceId: string, title: string, origin: 'workspace' | 'manual'): SourceRemovalPending {
  ...
  return { sourceId, title, cardCount, contradictionCount, origin }
}
...
export function decideSourceRemoval(
  sourceId: string,
  action: 'delete' | 'keep'
): { deletedItems: number; deletedContradictions: number } {
  const p = pending.get(sourceId)
  if (!p) {
    logMain('workspace', '处理来源移除确认 source=' + sourceId + ' 不在待确认列表，跳过')
    return { deletedItems: 0, deletedContradictions: 0 }
  }
  ...
  logMain('workspace', '处理来源移除确认 source=' + sourceId + ' action=' + action + ' 删除卡片=' + result.deletedItems + ' 矛盾=' + result.deletedContradictions)
  return result
}
