...
- **进行中**：三段式撰写重构 Phase 6.0–6.6 已实现（数据模型/汇编生成/三步向导/生成链路/删 skills+默认规范/规范库/范本/三套主题/编辑器深改；Migration 016–020）。Phase 6.8「按步骤分别指定大模型」已完成：设置页全局第 1/3 步默认模型，且**大模型配置一律以步骤默认模型为准**（已删全局「默认大模型」设置与任务级输入框上方的大模型下拉；重新生成初稿修复无反馈问题并确认走第 3 步默认模型）。验证：typecheck 零错误、166 项单测、生产构建通过。**待用户实测：真实 Provider 的 AI 细读/矛盾标注、三步端到端、基于汇编生成初稿、按步骤默认模型生效；随后进入 Phase 6.7（测试/文档/发布）与 Phase 5（Windows 安装包/端到端演示）。**
