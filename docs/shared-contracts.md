...
| `compilation:webMaterials` | `{ taskId }` → `{ pinned }` | **只读** 已锁定的网页材料篇数。面板据此显示「本任务已锁定 N 篇」，重启软件后同样可见（不依赖"本次会话生成过一次"） |

> **网页材料集合与抓取上限（第三批 A1，2026-09-12）**：首次生成把本轮**实际采用**的网页来源锁定进 `task_web_materials`（Migration 039，主键 `(task_id, source_id)`，两个外键 `ON DELETE CASCADE`）；此后「重新生成汇编」默认**复用同一批**（`listPinnedWebMaterials` 并回检索范围），站点上的新命中文章只统计数量（`webScan.reused` / `newCandidates`）。抓取上限（`WEB_FETCH_MAX_ARTICLES=300` / `WEB_FETCH_MAX_CHARS=1500000`）**只是成本保险丝**：只要上限小于标题粗筛后的候选数（真实站点同一主题实测 495–3,293 篇），按标题排序必然丢掉"标题不含主题词、正文却切题"的文章（如《融侨国际双语学校奠基仪式举行》），相关性取舍最终由**正文**承担（抓取时正文精过滤 + 保守闸门 + 细读窗口）。排序 `rankArticlesByQuery` 的打分口径必须与 `matchesAny` 一致（**bigram 重叠计数** + 完整检索词加权）：2026-09-12 实测旧口径（`title.includes(term)`，而检索词是长词/空格拼接的关键词串）对真实站点 477 篇与 80 篇标题**全部 0 分**，排序失效、上限退化成"按清单顺序截断"，汇编网页段落由 77 段掉到 4 段。
> **网页正文质量：A1/A2 抓取端 + C 段落端（2026-09-12 用户实测后实施）**：`WebFetchStats` 新增 `invalidBody`（抓回来发现"没取到正文"——老文章链接已失效、站点返回 HTTP 200 + 通用模板页——而丢弃的篇数，进生成汇总）；`CompilationExtractScan` 新增 `titleOnlyDropped`（段落只是复述来源标题、没有正文信息而被丢弃的段数）与 `timeUnsupported`（年份在来源里查不到也推不出、已降级为「时间待核」的段数）；`Source` / `sources` 表新增 `textSource`（`extractor` | `full-page`，正文来源）与 `bodyMissing`（正文缺失标记，不参与检索、不重试索引）；`RagIndexStatusRes` 新增 `bodyMissing`（设置页显示「另有 N 篇网页材料未取到正文」）。**Migration 040** 加这两列，**Migration 041** 对存量数据做一次性清理：正文里找不到标题前 8 字的**任务绑定**网页来源 → 标 `body_missing=1`、清空其向量、从 `task_web_materials` 摘掉（**不删 sources 行**：汇编段落经外键引用来源，删行会级联删掉已生成的段落）。
> **已删除：`compilation:refreshWebMaterials` 与 `compilation:adoptWebMaterials`（2026-09-12 当日引入当日删除）**。两者都是"界面入口传原始撰写要求"的对外动作：标题粗筛用 `matchesAny`（bigram，宽松）能命中数百篇，正文精过滤 `matchesExact` 却要求**原样包含整个检索词**，而界面入口取到的是「高中学校设置」这类 **7 字长词** → 一篇都落不进来（实测「锁定 0 篇（标题命中 495 篇，因上限未采用 0 篇）」），且 `skippedByCap=0`、界面完全看不出是被精过滤丢的。`refreshWebMaterials` 还额外是"先清空锁定集合再抓取"的非原子设计（失败即丢掉旧集合）。**生成管线不受影响**：它传的是大模型提取的**短关键词**。网页材料集合自此只由生成管线自身管理（首次落定 + 后续复用），面板只显示「已锁定 N 篇」。
> **补齐「重新生成汇编」入口（2026-09-12）**：已有汇编的任务此前**没有任何重跑生成的入口**（悬浮面板在已有汇编时只提供「与汇编对话」，`compilation.regenerateBtn` 是没有引用的死文案），用户只能新建任务。现于「与汇编对话」面板底部新增**「重新生成汇编」**（二次确认后调用同一条 `compilation:generate` 管线）——网页材料沿用 A1 已锁定的那一批（不重新抓取），生成结果是一版**新的汇编**（新 id、新对话记录与版本历史；旧版留在库中但界面只显示最新一版）。
