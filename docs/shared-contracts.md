...
| `compilation:adoptWebMaterials` | `{ taskId, query }` → `{ added, skippedByCap, siteErrors }` | **第三批 A1** 纳入新网页材料：站点上"新命中但未纳入"的文章由用户显式抓取并锁定到本任务（不动已有汇编） |
| `compilation:refreshWebMaterials` | `{ taskId, query }` → `{ sites, siteErrors, hits, fetched, skippedByCap, chars, pinned, cleared }` | **第三批 A1 补强** 重新检索网页材料：清空本任务锁定集合 → 重新发现/排序/抓取（含上限）→ 重新锁定。首次落定的材料不理想时不必新建任务；只改材料集合，需再点「重新生成汇编」才生效 |
| `compilation:webMaterials` | `{ taskId }` → `{ pinned }` | **第三批 A1 补强（只读）** 已锁定的网页材料篇数。面板据此**不依赖生成**也能显示「本任务已锁定 N 篇」与「重新检索网页材料」入口（重启软件后同样可见——此前该提示条只长在生成模式里，而重新生成会先按旧集合复用，导致入口事实上够不到） |

> **网页材料集合与抓取上限（第三批 A1 / A1 补强，2026-09-12）**：首次生成把本轮**实际采用**的网页来源锁定进 `task_web_materials`（Migration 039，主键 `(task_id, source_id)`，两个外键 `ON DELETE CASCADE`）；此后「重新生成汇编」默认**复用同一批**（`listPinnedWebMaterials` 并回检索范围），站点上的新命中文章只统计数量（`webScan.reused` / `newCandidates`），由用户点「纳入新材料」或「重新检索网页材料」才改变集合。抓取上限（`WEB_FETCH_MAX_ARTICLES=300` / `WEB_FETCH_MAX_CHARS=1500000`）**只是成本保险丝**：只要上限小于标题粗筛后的候选数（真实站点同一主题实测 495–3,293 篇），按标题排序必然丢掉"标题不含主题词、正文却切题"的文章（如《融侨国际双语学校奠基仪式举行》），相关性取舍最终由**正文**承担（抓取时正文精过滤 + 保守闸门 + 细读窗口）。排序 `rankArticlesByQuery` 的打分口径必须与 `matchesAny` 一致（**bigram 重叠计数** + 完整检索词加权）：2026-09-12 实测旧口径（`title.includes(term)`，而检索词是长词/空格拼接的关键词串）对真实站点 477 篇与 80 篇标题**全部 0 分**，排序失效、上限退化成"按清单顺序截断"，汇编网页段落由 77 段掉到 4 段。
